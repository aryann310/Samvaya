...
test('DigiLocker Integration Test Suite', async (t) => {
  ...
  await t.test('3. Step 2.1: Initiate DigiLocker verification request', async () => {
    ...
    assert.ok(
      result.consentUrl.includes('/digilocker/mock-consent'),
      'Sandbox must use local mock consent URL'
    );
  ...
  await t.test('4. Step 2.2: Callback CSRF validation (Security Test)', async () => {
    ...
    await assert.rejects(
      ...
      /Security Error: Invalid or mismatched state token/,
      'Must reject forged state token with CSRF security error'
    );
  ...
  await t.test('6. Step 2.2 - 2.4: Successful verification, document retrieval & automatic token revocation', async () => {
    ...
    assert.equal((storedRecord as any)?.accessToken, undefined, 'Access token must never be in storage');
  });
