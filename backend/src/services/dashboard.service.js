...
export class DashboardService {
    static getDashboard(businessId) {
        ...
        return {
            ...
            stats: {
                revenue: { value: latestRev, trend: revTrend, data: financials.map((f) => f.revenue?.total || 0) },
                expenses: { value: latestExp, trend: expTrend, data: financials.map((f) => f.expenses?.total || 0) },
                profit: { value: latestProfit, trend: profitTrend, data: financials.map((f) => f.profit || 0) },
            ...
            cashFlowChart: cashFlowEntries.map((c) => ({
                month: c.month,
