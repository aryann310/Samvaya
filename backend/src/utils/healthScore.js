export function calculateHealthScore(margin, cashRunwayMonths, dti, inventoryTurnover, revGrowth) {
    let marginScore = 0;
    if (margin > 20)
        marginScore = 100;
    else if (margin >= 15)
    ...
    if (dti === 0)
        ...
        dtiScore = 20;
    let turnoverScore = 0;
    if (inventoryTurnover === 'high')
        turnoverScore = 100;
    else if (inventoryTurnover === 'medium')
        turnoverScore = 60;
    else
        turnoverScore = 30;
    let revScore = 0;
    ...
    if (score >= 80)
        ...
        explanation = 'Good health, but room for improvement.';
    else
        explanation = 'Needs immediate attention.';
