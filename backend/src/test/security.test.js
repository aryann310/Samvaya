...
import { maskPAN, maskAadhaar, maskPhone, maskBankAccount, maskIFSC, redactSensitiveData, redactString, } from '../security/redact.js';
...
assert(encryptedRecord.owner.bankAccount.accountNumber.startsWith(ENVELOPE_PREFIX), 'Account number must be encrypted in DAL');
...
hstsMiddleware({}, mockRes, () => { });
assert.strictEqual(mockRes.headers['Strict-Transport-Security'], 'max-age=31536000; includeSubDomains; preload', 'HSTS header must be set with 1 year max-age and preload');
